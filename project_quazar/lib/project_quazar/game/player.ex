defmodule Player do
  ...
  @doc 'Returns whether this player is alive or not. (> 0 health, or ship destroyed)'
  ...
  defimpl Movable.Motion, for: __MODULE__ do
    ...
    @doc 'Rotates ship clockwise in degrees (Turns right)'
    def rotate(%@for{ship: ship} = player_data, rad, :cw) do
      %@for{ player_data | ship: Movable.Motion.rotate(ship, rad, :cw) }
    end

    @doc 'Rotates ship counter-clockwise in degrees (Turns left)'
